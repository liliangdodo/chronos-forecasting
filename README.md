...
docker exec -d chronos-train bash -c "cd /data/liliang/BXprojects/chronos-forecasting && PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 torchrun --nproc_per_node=8 --master_port=29501 scripts/training/train-2.py --config scripts/training/configs/chronos-2.yaml > train.log 2>&1"
...
torchrun --nproc_per_node=4 scripts/training/train-2.py --config scripts/training/configs/chronos-2.yaml 2>&1 | tee train.log
